...
class PesticideScraper:
        ...
        for i in range(2, lastpage + 2):  # lastpage + 1  |  2
            ...
            for row in rows:
                ...
                for c in col2:
                    ...
                    # extension_date = datetime.strptime(c.xpath('br')[0].tail.strip(), '%d/%m/%Y').date()
                    # # Check if the extension date falls within the specified date range
                    # if (start_date is None or start_date <= extension_date) and (end_date is None or extension_date <= end_date):
                    #     Extensions[auth_number] = extension

                col1 = row.xpath('td[1]')
                for c in col1:  #changes
    ...
    def getAdjuvantLinks(self, target):
        ...
        if target == "All":
            ...
            while ResponseStatus == 200:
                ...
                authorisations = parsed.xpath("//table[@id='ctl00_ContentPlaceHolder1_AdjuvantGV']//tr/td[3]")   #changes
                all_authorisation.extend(authorisations)
                
            return links, all_authorisation
    ...
    def getAdjuvantInfo(self, link):
        ...
        dictionary.update({'Name': parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblAdjuvantName']//text()")[0]})  #changes
        dictionary.update({'Number': parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblAdjuvantNumber']//text()")[0]})
        dictionary.update({'Formulation':
                               parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblFormulation']/text()")[0].split(
                                   'containing')[0]})
        dictionary.update({'Applicant': str(parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblApplicant']//text()")[0]).split(',')[0]})
        dictionary = self.check_date(parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblIncDate']//text()")[0], dictionary, 'Date of Inclusion')
        dictionary.update({'Field of Use': parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblFOU']//text()")[0]})
        ...
        dictionary.update({'List Entry - Document': parsed.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblListEntryNumber']//text()")[0].split('. ')[1]})
        ...
        if args.adjuvant:
            try:
                ...
                # Perform entire extraction
                    ...
                    # Get new records link list
                    links, authorisations = scraper.getAdjuvantLinks('All') #changes
                    print(len(links), "Adjuvant records to be extracted...")
                    print(len(authorisations), "Total authorisation")
                    adjuvants = {}
                    print(type(authorisations))
                    # Get information from Details Page
                    for cont, link in enumerate(links):
                        print("\t" + str(cont) + "/" + str(len(links)))
                        adjuvant = scraper.getAdjuvantInfo(link)
                        adjuvant.update({"Extent of Authorisation": authorisations[cont].text}) #changes
                        adjuvants[adjuvant['Number']] = adjuvant
